...
definition(
    ...
    author: "SmartThings",
		description: "Turn things off if you're using too much energy and back on when it stops",
    category: "My Apps",
...
def meterHandler(evt) {
    ...
    if (!state.cycleOn && meterValue > thresholdValue) {
	    log.debug "${meter} reported energy consumption above ${threshold}. Turning ${switches} on."
    	state.cycleOn = true
        switches.on()
        }
    else if (state.cycleOn && meterValue <= thresholdValue) {
		log.debug "${meter} reported energy consumption below ${threshold}. Turning ${switches} off."
        state.cycleOn = false
        switches.off()
        }
      }
