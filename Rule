                ...
                section((state.isTrig || isTrig) ? "Event Trigger #$i" : "Condition #$i") {                
                    ...
                    if(myCapab) {
                        ...
                        if(!(xCapab in excludes)) {
                            ...
                            if(xCapab in ["Temperature", "Humidity", "Illuminance", "Dimmer level", "Energy meter", "Power meter", "Battery"]) {
                                supportsQuietTime = false
                                getRelational(thisDev)
                            }
                        } else if(xCapab == "Button") {
                            supportsQuietTime = false
                            getButton(isTrig ? "tDev$i" : "rDev$i")
                        }                    
                        getState(xCapab, i, isTrig)
                        if (supportsQuietTime) {
                            def myQuiet = isTrig ? "tquiet$i" : "quiet$i"
                            input myQuiet, "number",     title: "Only if no change happened in the last these minutes",    range: "0..1440", required: false
                        }
                    }
                }
            }
...
def testEvt(evt) {
        log.trace "testEvt(" + evt + ")"
        log.debug "settings = $settings"

        def result = false
        def howMany = state.isTrig ? state.howMany : state.howManyT
        if(evt.name == "mode") {
            log.debug "modesX is ${modesX} and evt.value is ${evt.value}"
            return (!modesX || modesX.contains(evt.value))
        }
        ...
        for(int i = 1; i < howMany; i++) {
            ...
            def myQuiet = settings.find {it.key == (state.isTrig ? "quiet$i" : "tquiet$i")}
            if(myDev) {
                myDev = myDev.value
                myDev.each {if(evt.displayName == it.displayName) {
                    ...
                    if (result) {
                        log.debug "PREPARING TO CHECK QUIET TIME FOR EVENT " + evt
                        result = result && checkQuietTime(evt.device, myQuiet ? myQuiet.value : 0, evt.name)
                    }
                }
            }
...
def cmdActions(str, truth) {
 ...
 //added by ady624 to extend Rule to support quiet time
    def checkQuietTime(dev, quietX, attr) {
        log.debug "CHECKING QUIET TIME FOR DEVICE " + dev + " WHERE QUIET TIME IS " + quietX + " and attr is $attr"
        //we return true if no quiet time is required
        if (!quietX) {
            return true
        }
        log.debug "attr is $attr, dev is $dev"
        if (attr) {
            //get events in the last {quietX} minutes
            def events = dev.eventsSince(new Date(now() - quietX * 60000));
            //if we got any events, let's go through them       
            //we start at the second event, as the first one is the event that triggered this function. The attribute's value has to be different from the current one to qualify for quiet        
            log.debug "Checking for quiet time, event count is " + events.size()
            for(def i = 1; i < events.size(); i++) {
                log.debug "event $i is ${events[i].name}"
                if (events[i].name == attr) {
                    //we found an event for the same attribute, so we have noise
                    log.debug "Event is reocurring within the quiet time interval, ignoring event"
                    return false;
                }
            }
