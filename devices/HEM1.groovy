 ...
 *  02/19/2016 : Updated UI tiles
 */
metadata {
    definition (name: "Aeon HEM", namespace: "BD", author: "SmartThings") 
{
...
// tile definitions
	tiles(scale: 2) {
		multiAttributeTile(name:"powerDisp", type: "lighting", width: 6, height: 4, decoration: "flat", canChangeIcon: true, canChangeBackground: true){
            ...
            tileAttribute ("click", key: "SECONDARY_CONTROL") {
           		attributeState "default", label:'Click to Refresh' 
            }     
        }    
        ...
        standardTile("energyOne", "device.energyOne", width: 4, height: 2, inactiveLabel: false, decoration: "flat") {
            state("default", label: '${currentValue}', backgroundColor:"#ffffff")
        }        
        valueTile("energyTwo", "device.energyTwo", width: 3, height: 1, inactiveLabel: false, decoration: "flat") {
            state("default", label: '${currentValue}', backgroundColor:"#ffffff")
        }
        standardTile("click", "click", width: 6, height: 2, inactiveLabel: false) {
            state("click", label: 'Click to Refresh Data', backgroundColor:"#ffffff")
        }

    	standardTile("refresh", "device.power", width: 1, height: 2, inactiveLabel: false, decoration: "flat") {
        ...
        standardTile("statusText", "statusText", width: 6, height: 2, inactiveLabel: false, decoration: "flat") {
            state "statusText", label:'${currentValue}', backgroundColor:"#ffffff"
        }

        standardTile("min", "powerOne", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
            state "default", label:'Min:\n${currentValue}', backgroundColor:"#ffffff"
        }

        standardTile("max", "powerTwo", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
        ...
        main (["powerDisp"])
        details(["powerDisp", "energyDisp", "energyTwo", "resetmaxmin", "reset", "statusText", "energyOne", "configure"])
        }
...
def reset() {
	...
	def timeString = new Date().format("yyyy-MM-dd h:mm a", location.timeZone)
    sendEvent(name: "energyOne", value: "Last Energy Reset:\n"+timeString, unit: "")       
...
def resetmaxmin() {
	...
	def timeString = new Date().format("yyyy-MM-dd h:mm a", location.timeZone)
    sendEvent(name: "energyOne", value: "Last Watts Reset:\n"+timeString, unit: "")
