 ...
 *
 *  Notify with Hue Settings
 *
 *  Author: SmartThings
 *  Date: 2015-12-09
 */
definition(
    name: "Notify with Hue Settings",
...
dynamicPage(name: "pageSetup", install: true, uninstall: true) {
    ...
    section("Name your notification") {
    		label title:"Notification Name", required:true
    	}
	...
	section("Choose light effects...") {
		 ...
		 input "flasher", "bool", title: "Flash Lights?", required: false, defaultValue: false,  submitOnChange:true
	}
    ...
    if (flasher) {
		...
		section(" Flashing times (optional - in milliseconds )..."){
...
private flashLights() {
	...
	if (doFlash) {
		...
		def initialActionOn = hues.collect{it.currentSwitch != "on"}
		def delay = 1L
		
        numFlashes.times {
			log.trace "hues on after  $delay msec"
			...
			delay += onFor
			log.trace "hues off after $delay msec"
			hues.eachWithIndex {s, i ->
				if (initialActionOn[i]) {
					s.off(delay: delay)
				}
